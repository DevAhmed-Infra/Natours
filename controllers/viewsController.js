...
exports.getOverview = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log(`[getOverview] ✓ Query success: Found ${tours ? tours.length : 0} tours`);
  } catch (err) {
  ...
  console.log(`[getOverview] Data being passed: { title: 'All Tours', tours: Array(${tours.length}) }`);
...
exports.getTour = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log(`[getTour] ✓ Query success: Tour found = ${tour ? tour.name : 'Not found'}`);
  } catch (err) {
    console.error('[getTour] ✗ Query failed:', err.message);
    return next(new AppError('Error fetching tour', 500));
  }
...
exports.getLoginForm = (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ status: 'error', message: 'Failed to render login template' });
  }
...
exports.getAccount = (req, res) => {
  ...
  console.log(`[getAccount] User logged in: ${req.user ? req.user.name : 'No user'}`);
  ...
  try {
    ...
    res.status(500).json({ status: 'error', message: 'Failed to render account template' });
  }
...
exports.updateUserData = asyncHandler(async (req, res, next) => {
  ...
  console.log(`[updateUserData] Update data: { name: '${req.body.name}', email: '${req.body.email}' }`);
  ...
  try {
    ...
    console.log(`[updateUserData] ✓ Update success: ${updatedUser ? updatedUser.name : 'Failed'}`);
  } catch (err) {
